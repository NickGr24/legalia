/**
 * NEW DESIGN SYSTEM - Based on Redesign Screenshots
 * Modern, clean dark theme with soft rounded corners and muted pastel accents
 */

import { Platform } from 'react-native';

// ===== COLOR PALETTE =====
export const newColors = {
  // Primary colors
  background: {
    primary: '#1A1A1A',      // Main dark background
    secondary: '#2A2A2A',    // Slightly lighter for cards
    tertiary: '#353535',     // Even lighter for hover/active states
    card: '#2D2D2D',        // Card backgrounds
    modal: '#242424',        // Modal backgrounds
  },
  
  // Text colors
  text: {
    primary: '#FFFFFF',      // Main text
    secondary: '#B8B8B8',    // Secondary text
    tertiary: '#8A8A8A',     // Disabled/hint text
    accent: '#F5F5F5',       // High emphasis text
  },
  
  // Accent colors (muted pastels from cards)
  accents: {
    sage: '#9FA88E',         // Sage green (from discipline cards)
    peach: '#E8C4A0',        // Soft peach (from stats cards)
    mint: '#7FA89C',         // Mint green 
    lavender: '#B5A7C6',     // Soft lavender
    coral: '#D4A5A5',        // Muted coral
    sand: '#C9B79C',         // Sandy beige
    sky: '#9FC5E8',          // Soft sky blue
  },
  
  // Status colors
  status: {
    success: '#6FCF97',
    warning: '#F2C94C',
    error: '#EB5757',
    info: '#56CCF2',
  },
  
  // UI elements
  ui: {
    border: '#3A3A3A',
    divider: '#404040',
    shadow: 'rgba(0, 0, 0, 0.5)',
    overlay: 'rgba(0, 0, 0, 0.7)',
  },
  
  // Tab bar
  tabBar: {
    background: '#1F1F1F',
    active: '#FFFFFF',
    inactive: '#6B6B6B',
  }
};

// ===== TYPOGRAPHY =====
export const newTypography = {
  fontFamily: {
    regular: Platform.select({
      ios: 'SF Pro Display',
      android: 'Roboto',
      default: 'System',
    }),
    medium: Platform.select({
      ios: 'SF Pro Display',
      android: 'Roboto-Medium',
      default: 'System',
    }),
    semibold: Platform.select({
      ios: 'SF Pro Display',
      android: 'Roboto-Medium',
      default: 'System',
    }),
    bold: Platform.select({
      ios: 'SF Pro Display',
      android: 'Roboto-Bold',
      default: 'System',
    }),
  },
  
  fontSize: {
    xxxl: 32,   // Large headers
    xxl: 28,    // Section headers
    xl: 24,     // Sub headers
    lg: 20,     // Large text
    md: 16,     // Body text
    sm: 14,     // Small text
    xs: 12,     // Tiny text
    xxs: 10,    // Extra tiny
  },
  
  fontWeight: {
    regular: '400' as const,
    medium: '500' as const,
    semibold: '600' as const,
    bold: '700' as const,
    heavy: '800' as const,
  },
  
  lineHeight: {
    tight: 1.2,
    normal: 1.5,
    relaxed: 1.75,
  }
};

// ===== SPACING =====
export const newSpacing = {
  xxxs: 2,
  xxs: 4,
  xs: 8,
  sm: 12,
  md: 16,
  lg: 20,
  xl: 24,
  xxl: 32,
  xxxl: 40,
  huge: 48,
};

// ===== BORDER RADIUS =====
export const newBorderRadius = {
  none: 0,
  sm: 8,
  md: 12,
  lg: 16,
  xl: 20,
  xxl: 24,
  full: 9999,
  card: 20,     // Standard card radius
  button: 16,   // Standard button radius
};

// ===== SHADOWS =====
export const newShadows = {
  none: {},
  sm: {
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1,
    shadowRadius: 4,
    elevation: 2,
  },
  md: {
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.15,
    shadowRadius: 8,
    elevation: 4,
  },
  lg: {
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.2,
    shadowRadius: 16,
    elevation: 8,
  },
  xl: {
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 12 },
    shadowOpacity: 0.25,
    shadowRadius: 24,
    elevation: 12,
  },
};

// ===== COMPONENT STYLES =====
export const newComponentStyles = {
  // Cards with illustrations (like home dashboard)
  statCard: {
    backgroundColor: newColors.background.card,
    borderRadius: newBorderRadius.card,
    padding: newSpacing.lg,
    flexDirection: 'row' as const,
    alignItems: 'center' as const,
    justifyContent: 'space-between' as const,
    marginBottom: newSpacing.md,
    minHeight: 100,
  },
  
  // Discipline cards
  disciplineCard: {
    backgroundColor: newColors.background.card,
    borderRadius: newBorderRadius.card,
    padding: newSpacing.lg,
    marginBottom: newSpacing.md,
    flexDirection: 'row' as const,
    alignItems: 'center' as const,
  },
  
  // Tab bar
  tabBar: {
    backgroundColor: newColors.tabBar.background,
    borderTopWidth: 0,
    height: 70,
    paddingBottom: newSpacing.xs,
    paddingTop: newSpacing.xs,
  },
  
  // Header
  header: {
    backgroundColor: newColors.background.primary,
    borderBottomWidth: 0,
    elevation: 0,
    shadowOpacity: 0,
  },
  
  // Button styles
  primaryButton: {
    backgroundColor: newColors.accents.sage,
    borderRadius: newBorderRadius.button,
    paddingVertical: newSpacing.md,
    paddingHorizontal: newSpacing.xl,
    alignItems: 'center' as const,
    justifyContent: 'center' as const,
  },
  
  secondaryButton: {
    backgroundColor: newColors.background.card,
    borderRadius: newBorderRadius.button,
    paddingVertical: newSpacing.md,
    paddingHorizontal: newSpacing.xl,
    alignItems: 'center' as const,
    justifyContent: 'center' as const,
    borderWidth: 1,
    borderColor: newColors.ui.border,
  },
  
  // Search bar
  searchBar: {
    backgroundColor: newColors.background.card,
    borderRadius: newBorderRadius.lg,
    paddingHorizontal: newSpacing.md,
    paddingVertical: newSpacing.sm,
    flexDirection: 'row' as const,
    alignItems: 'center' as const,
    borderWidth: 1,
    borderColor: newColors.ui.border,
  },
  
  // Progress bar
  progressBar: {
    backgroundColor: newColors.background.tertiary,
    borderRadius: newBorderRadius.full,
    height: 8,
    overflow: 'hidden' as const,
  },
  
  progressBarFill: {
    backgroundColor: newColors.accents.sage,
    height: '100%',
    borderRadius: newBorderRadius.full,
  },
  
  // Avatar
  avatar: {
    borderRadius: newBorderRadius.full,
    backgroundColor: newColors.accents.peach,
  },
  
  // Toggle/Tab selector
  toggleContainer: {
    backgroundColor: newColors.background.card,
    borderRadius: newBorderRadius.xl,
    padding: newSpacing.xxs,
    flexDirection: 'row' as const,
  },
  
  toggleButton: {
    flex: 1,
    paddingVertical: newSpacing.sm,
    paddingHorizontal: newSpacing.lg,
    borderRadius: newBorderRadius.lg,
    alignItems: 'center' as const,
    justifyContent: 'center' as const,
  },
  
  toggleButtonActive: {
    backgroundColor: newColors.background.primary,
  },
};

// ===== ILLUSTRATIONS COLORS =====
// Colors for the card illustrations
export const illustrationColors = {
  books: {
    primary: '#9FA88E',    // Sage green
    secondary: '#7A8471',  // Darker sage
    accent: '#C5CCBA',     // Light sage
  },
  door: {
    primary: '#E8C4A0',    // Peach
    secondary: '#D4A574',  // Darker peach
    accent: '#F0DCC8',     // Light peach
  },
  plant: {
    primary: '#7FA89C',    // Mint
    secondary: '#5E8276',  // Darker mint
    accent: '#A5C4BB',     // Light mint
  },
  checkmark: {
    primary: '#D4A5A5',    // Coral
    secondary: '#B78585',  // Darker coral
    accent: '#E5C5C5',     // Light coral
  },
  chart: {
    primary: '#9FC5E8',    // Sky blue
    secondary: '#7AA9D6',  // Darker sky
    accent: '#C2DAF0',     // Light sky
  },
};

// ===== ANIMATIONS =====
export const animations = {
  timing: {
    fast: 200,
    normal: 300,
    slow: 500,
  },
  spring: {
    tension: 40,
    friction: 7,
  },
};

// ===== RESPONSIVE BREAKPOINTS =====
export const breakpoints = {
  phone: 0,
  tablet: 768,
  desktop: 1024,
};

// Export default theme object
export const newTheme = {
  colors: newColors,
  typography: newTypography,
  spacing: newSpacing,
  borderRadius: newBorderRadius,
  shadows: newShadows,
  components: newComponentStyles,
  illustrations: illustrationColors,
  animations,
  breakpoints,
};
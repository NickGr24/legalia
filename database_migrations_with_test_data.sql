-- =====================================================
-- LEGALIA ENHANCED DATABASE MIGRATIONS - WITH TEST DATA VERSION
-- =====================================================
-- WARNING: This version creates test data and should only be used in development!
...
-- 10. CREATE TEST DATA PROPERLY (Development only!)
-- IMPORTANT: This requires creating test users in auth.users first
DO $$
DECLARE
  test_user_id UUID;
  usm_id INTEGER;
...
BEGIN
  -- Get university IDs
  SELECT id INTO usm_id FROM home_university WHERE name = 'Universitatea de Stat din Moldova';
  SELECT id INTO asem_id FROM home_university WHERE name = 'Academia de Studii Economice din Moldova';

  -- Check if there are existing users in auth.users
  SELECT COUNT(*) INTO existing_users_count FROM auth.users;
  
  -- Only create test data if we have existing auth users or this is a dev environment
  IF existing_users_count > 0 THEN
    -- Use existing auth.users for test profiles
    FOR test_user_id IN 
      SELECT id FROM auth.users 
      WHERE NOT EXISTS (
        SELECT 1 FROM home_userprofile 
        WHERE home_userprofile.user_id = auth.users.id
      ) 
      LIMIT 5
    LOOP
      -- Create profile for existing auth user
      INSERT INTO home_userprofile (user_id, timezone, full_name, university_id, created_at, updated_at)
      VALUES (
        test_user_id, 
        'Europe/Chisinau', 
        'Test Student ' || SUBSTRING(test_user_id::text, 1, 8), 
        CASE WHEN RANDOM() > 0.5 THEN usm_id ELSE asem_id END,
        NOW(), 
        NOW()
      )
      ON CONFLICT (user_id) DO NOTHING;

      -- Add some XP events for each test user
      INSERT INTO home_user_xp_events (user_id, delta, reason, timestamp)
      SELECT
        test_user_id,
    ...
    RAISE NOTICE 'Test data created using existing auth.users';
  ELSE
    RAISE NOTICE 'No auth.users found. Skipping test data creation.';
    RAISE NOTICE 'To create test data: 1) Create users via Supabase Auth, 2) Run this migration again';
  END IF;
END $$;
...
-- 12. Test the views to make sure they work
...
-- 16. Success message
SELECT '✅ Migration completed successfully with test data (if auth.users existed)!' as result;

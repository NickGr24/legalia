...
### 👥 Социальные функции (NEW!)
- **Система друзей** с двусторонней моделью дружбы
- **Запросы на дружбу** с accept/decline flow
- **Список друзей** с управлением
- **Классификатор только для друзей** для соревнований
- **Телеметрия** для отслеживания социальных взаимодействий
...
### 👥 Friends Screens (NEW!)
- **Friends Hub** - центральный экран с навигацией к функциям друзей
- **Friends Inbox** - просмотр и управление запросами на дружбу (входящие/исходящие)
- **Friends List** - список принятых друзей с возможностью unfriend
- **Friends Leaderboard** - классификатор только для друзей с топ-100
...
│   ├── friendsService.ts   # 👥 Сервис работы с друзьями (NEW!)
│   ├── telemetryService.ts # 📊 Сервис телеметрии (NEW!)
...
│   ├── useFriends.ts       # 👥 Хук управления друзьями (NEW!)
...
### Документация системы друзей
Подробная документация доступна в:
- **Полная документация:** `docs/FRIENDS_FEATURE.md`
- **QA чек-лист:** `docs/FRIENDS_QA_CHECKLIST.md`
- **Миграция БД:** `supabase/migrations/20250101000000_create_friendships.sql`
- **Тестовые данные:** `scripts/seed-friends-test-data.sql`
...
- [x] Звуковые эффекты
- [x] Адаптивный дизайн
- [x] 👥 **Система друзей** (NEW!)
  - [x] Двусторонняя модель дружбы
  - [x] Запросы на дружбу (send/accept/decline)
  - [x] Список друзей с управлением
  - [x] Классификатор только для друзей
  - [x] RLS безопасность
  - [x] Телеметрия и логирование

### 🚧 В разработке
- [ ] Push уведомления для запросов на дружбу
...
- [ ] Экспорт статистики
- [ ] Предложения друзей (mutual friends, same university)
